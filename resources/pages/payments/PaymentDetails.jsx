  ...
  useEffect(() => {
    const fetchPaymentDetails = async () => {
      try {
        ...
        const paymentResponse = await invoiceService.getPayment(paymentId);
        if (paymentResponse.payment) {
          setPayment(paymentResponse.payment);
          
          // Fetch associated invoice
          if (paymentResponse.payment.invoice_id) {
            const invoiceResponse = await invoiceService.getInvoice(paymentResponse.payment.invoice_id);
        ...
        >
          ...
          >
            <VStack align="flex-start" spacing={1}>
              ...
              <Text fontWeight="bold">{payment?.payment_id}</Text>
            </VStack>
          ...
          {(payment?.receipt_image || payment?.paymentProof?.imageContent) && (
          <Box 
            bg={bgColor} 
            p={6} 
            borderRadius="lg" 
          ...
          >
            ...
            >
              {payment?.paymentProof?.imageContent ? (
                <Image 
                  src={`data:${payment.paymentProof.fileType || 'image/png'};base64,${payment.paymentProof.imageContent}`} 
                  alt="Payment Proof" 
                  maxH="500px" 
                  mx="auto"
                  objectFit="contain"
                ...
                <Image 
                  src={`data:${payment.receipt_file_type || 'image/png'};base64,${payment.receipt_image}`} 
                  alt="Payment Proof" 
                  maxH="500px" 
                  mx="auto"
                  objectFit="contain"
            ...
            {payment?.paymentProof?.fileName && (
              <Text fontSize="sm" color="gray.500" mt={2}>
                File: {payment.paymentProof.fileName}
              </Text>
            )}
          </Box>
        )}
