...
const InvoiceDetail = () => {  const { invoiceId } = useParams();
  ...
  useEffect(() => {
    const fetchInvoiceData = async () => {
      try {
        ...
        // Fetch tenant payments to get full payment proof data
        try {
          const paymentsResponse = await paymentService.getTenantPayments(tenantId);
          
          if (paymentsResponse && paymentsResponse.payments) {
            // Filter payments for this invoice
            const invoicePayments = paymentsResponse.payments.filter(payment => 
              payment.invoiceId === parseInt(invoiceId)
            );
            ...
            if (invoicePayments.length > 0) {
              ...
              console.log('Updated invoice with payments:', updatedInvoice);
              
              // Check for pending manual payments
              const pendingManualPayments = invoicePayments.filter(payment => 
          ...
          console.warn('Could not fetch tenant payments:', error);
        }
        
        // Fetch related booking data if booking_id exists
        if (mappedInvoice.booking_id || mappedInvoice.bookingId) {          try {
            const bookingResponse = await fetch(`/v1/bookings/${mappedInvoice.booking_id || mappedInvoice.bookingId}`, {
              headers: {
            ...
            if (bookingResponse.ok) {
              ...
              if (tenantId && bookingData.booking) {
                try {
                  const tenantResponse = await fetch(`/v1/tenants/${tenantId}`, {
                    headers: {
                      'Authorization': `Bearer ${localStorage.getItem('token')}`,
                      'Content-Type': 'application/json'
                    }
                  ...
                  if (tenantResponse.ok) {
                    ...
                    console.warn('Tenant fetch failed, using existing booking data');
                ...
                <VStack align="start" spacing={3}>
                  ...
                  <Box>
                    ...
                    <Badge 
                      ...
                      variant="solid"
                    >
                      {tenantData.tenant_type?.name || tenantData.type?.name || 'N/A'}
                    </Badge>
                  </Box>
                  {tenantData.address && (
                    <Box>
          ...
          <Box mb={4}>
            ...
            <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
              <Box>
                ...
                <Text fontSize="sm" color="gray.600">
                  Classification: {bookingData?.room?.classification?.name || invoice.booking?.room?.classification?.name || 'N/A'}
                </Text>
                <Text fontSize="sm" color="gray.600">
                  Rental Type: {bookingData?.room?.rentalType?.name || invoice.booking?.room?.rentalType?.name || 'N/A'}
                </Text>
              </Box>
              <Box>
                ...
                <Text fontSize="sm" color="gray.600">Rate per period</Text>
                <Text fontWeight="medium">
                  {formatCurrency(bookingData?.room?.rate || invoice.booking?.room?.rate || 0)}
                </Text>
              </Box>
            </SimpleGrid>
          </Box>
